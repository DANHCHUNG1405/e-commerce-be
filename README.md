...
Seller Service (`cmd/seller`) tự chuyển `sellers` và `seller_members` sang schema `seller` sau migration lõi 0010. API proxy các endpoint cửa hàng/thành viên tại URL cũ, còn dashboard, đơn hàng và tồn kho vẫn do API xử lý. Các process khác lấy trạng thái, hoa hồng, membership và dữ liệu vận hành Seller qua gRPC; Chat dùng gRPC cho quyền hội thoại và người nhận realtime. RPC dữ liệu vận hành/người nhận yêu cầu service JWT ngắn hạn (`api`/`chat`); RPC theo người dùng yêu cầu access JWT. Checkout giữ order/kho/outbox trong một transaction và dùng một snapshot Seller cho mỗi seller trong checkout. PostgreSQL vẫn dùng chung và foreign key xuyên schema vẫn còn; thay đổi trạng thái Seller đồng thời với checkout có thể xảy ra sau khi snapshot được lấy.
