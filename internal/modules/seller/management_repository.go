...
func (r *ManagementRepository) AdminList(ctx context.Context, status string, p, l int) ([]AdminSeller, error) {
	items := []AdminSeller{}
	q := r.db.WithContext(ctx).Table("seller.sellers s").Select("s.*, u.id AS owner_id, u.email AS owner_email").Joins("LEFT JOIN seller.seller_members sm ON sm.seller_id=s.id AND sm.role='owner'").Joins("LEFT JOIN identity.users u ON u.id=sm.user_id").Where("s.deleted_at IS NULL")
	if status != "" {
